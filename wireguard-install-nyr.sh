...
new_client_setup () {
	...
	if grep -q 'fddd:2c4:2c4:2c4::1' /etc/wireguard/wg0.conf; then
		ips_v6=", fddd:2c4:2c4:2c4::$octet/128"
		ips_v6_client=", fddd:2c4:2c4:2c4::$octet/64"
	else
		ips_v6=""
		ips_v6_client=""
	fi
...
PresharedKey = $psk
AllowedIPs = 10.7.0.$octet/32$ips_v6
# END_PEER $client
EOF
...
[Interface]
Address = 10.7.0.$octet/24$ips_v6_client
DNS = $dns
PrivateKey = $key
...
if [[ ! -e /etc/wireguard/wg0.conf ]]; then
	...
	client="${unsanitized_client//[^a-zA-Z0-9_-]/_}"
	client="${client:0:15}"
	...
	if [[ "$use_boringtun" -eq 1 ]]; then
		...
		read -r -p "Should automatic updates be enabled for it? [Y/n]: " boringtun_updates
...
EOF
	...
	case "$option" in
		...
		2)
			...
			echo
			read -r -p "Confirm $client removal? [y/N]: " remove
		...
		3)
			echo
			read -r -p "Confirm WireGuard removal? [y/N]: " remove
			until [[ "$remove" =~ ^[yYnN]*$ ]]; do
				echo "$remove: invalid selection."
				read -r -p "Confirm WireGuard removal? [y/N]: " remove
			done
